...
Cliquez du droit sur la base de données movies dans Workbench et cliquez sur Drop Schema et validez pour supprimer la base de données.

Rouvrez le dump et réexécutez le script ou téléchargez ce dossier et décompressez le si vous ne l'avez plus.

Nous repartirons ici de la base de données sans les altérations effectuées dans le chapitre précédent.
...
Les fonctions d'agrégation sont essentielles en SQL pour résumer les données, permettant d'obtenir des informations telles que les moyennes, les totaux et les comptages à partir de grandes tables.
...
Nous allons commencer par voir les plus utilisées dans cette leçon, puis les plus avancées dans la leçon suivante.
AVG() - calcul de la moyenne

La fonction AVG() est utilisée pour calculer la valeur moyenne d'un ensemble de valeurs numériques, excluant les valeurs NULL.
...
COUNT(*) compte toutes les lignes, tandis que COUNT(colonne) compte les lignes où la colonne spécifiée n'est pas NULL.
...
La fonction COUNT(DISTINCT) est utilisée pour obtenir le nombre de valeurs uniques dans une colonne, excluant les valeurs NULL.
...
Par exemple, supposons que nous ayons une table user_logs avec une colonne user_id. Pour compter le nombre d'utilisateurs uniques ayant accédé à une application sur une période donnée nous pouvons faire :
...
Ces fonctions (qui sont des alias) calculent l'écart type pour un ensemble de valeurs, ce qui est utile pour analyser la dispersion des données autour de la moyenne.
...
Ces fonctions (qui sont des alias) calculent la variance des valeurs, une mesure de la dispersion similaire à l'écart type mais en utilisant le carré des écarts par rapport à la moyenne.
...
Ces fonctions agrègent les résultats d'une requête en un tableau JSON ou en un objet JSON, respectivement.

Par exemple, pour obtenir un tableau JSON des titres de films et un objet JSON des films avec leur revenu :
...
colonne1 dans le GROUP BY est la colonne selon laquelle les données seront regroupées, et fonction_agrégation(colonne2) applique une fonction d'agrégation sur les données regroupées.
...
Utilisez la fonction YEAR() que nous reverrons en détail pour obtenir l'année à partir d'une date.
...
Filtrer des données agrégées ave la clause HAVING
Définition

La clause HAVING est un outil puissant en SQL qui permet de filtrer les résultats d'une requête SELECT qui utilise la clause GROUP BY.
...
Le total pour chaque année aura un genre à NULL et le total global aura NULL dans les deux premières colonnes. Nous trions par années ASC pour le montrer.