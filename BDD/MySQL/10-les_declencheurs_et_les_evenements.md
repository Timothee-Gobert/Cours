...
Les déclencheurs (triggers) dans MySQL offrent un moyen puissant de réagir automatiquement à des événements spécifiques dans la base de données. 
Syntaxe

Déclaration de création : on commence par la commande CREATE TRIGGER suivie du nom que l'on souhaite donner au déclencheur.
...
Il est recommandé de préfixer le nom du déclencheur par l'événement : insert_nom, update_nom ou delete_nom pour s'y retrouver plus facilement.
Cas d'utilisation
...
Lorsqu'un membre de l'équipe d'un film (cast ou crew) est ajouté, nous pourrions vouloir mettre à jour automatiquement un compteur de films pour cette personne.
...
Notre objectif est d'ajouter une colonne last_modified à la table movie et créer un déclencheur qui met à jour ce champ à chaque fois qu'un film est mis à jour.

Notez bien que nous verrons la modification de table en détail dans un chapitre dédié.

Nous verrons qu'il est possible de faire cela sans déclencheur avec MySQL et de définir une option directement sur la colonne de la table mais c'est uniquement pour l'exemple.
Étape 1 : ajouter la colonne last_modified

Vous commencerez par ajouter une nouvelle colonne à votre table movie. Voici la requête SQL pour ajouter cette colonne :
...
NEW.last_modified fait référence à la nouvelle valeur de la colonne last_modified pour la ligne qui est en train d'être mise à jour.
...
Pour voir les déclencheurs définis dans une base de données MySQL, vous pouvez utiliser la commande SHOW TRIGGERS.

Cette commande affiche une liste des déclencheurs avec des informations détaillées telles que le nom du déclencheur, le type d'événement (INSERT, UPDATE, DELETE), le timing (BEFORE, AFTER), la table associée, et la définition du déclencheur (la commande SQL qui est exécutée).
...
SHOW TRIGGERS LIKE 'nom_de_table';
...
Les événements sont des tâches automatisées qui sont exécutées à des intervalles prévus ou à des moments spécifiques, similaires aux tâches cron dans les systèmes d'exploitation Unix.
...
Avant de créer des événements, assurez-vous que le planificateur d'événements de MySQL est activé (il l'est par défaut normalement). Vous pouvez vérifier cela avec la commande suivante :
...
Cet exemple crée un événement nommé nettoyage_hebdomadaire qui démarre le 1er janvier 2025 à minuit et s'exécute toutes les semaines. À chaque exécution, il supprime les enregistrements de plus d'un mois de la table table_temporaire.
Nettoyage quotidien
...
Dans cet exemple, nettoyage_journalier est le nom de l'événement, et il supprime les lignes de la table table_temporaire qui ont une date_creation avant le jour courant.
